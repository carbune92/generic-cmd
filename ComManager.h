...
class ComManager
{
  ...
  private:
  int parseRebootCmd(def::data_t::iterator itBegin, 
                     int& offsetNewCmd,
                     def::t_CmdTypes cmdType = def::e_CmdDefault);

  int parseWatchdogCmd(def::data_t::iterator itBegin,
                      int& offsetNewCmd,
                      def::t_CmdTypes cmdType = def::e_CmdDefault);

  int parsePingCmd(def::data_t::iterator itBegin, 
                   int& offsetNewCmd,
                   def::t_CmdTypes cmdType = def::e_CmdDefault);
